...
pl <- ggplot(mpg, aes(x = hwy, y = cty)) +
  geom_point()
pl
ggsave(filename = "./my_saved_plot.pdf", plot = pl) # "." means the "current" folder
...
# If you can't read it, try to suppress messages  and warnings
...
write_csv(x = hate_crimes,"crimes.csv") # you can write comma-separated and tab-separated files using write_csv
...
#Example 1: Quantitative vs Quantitative 
...
# variables at larger y-values, then try logging the y-variable instead of the 
...
# Same example as above but now use ggplot
...
# Example 2: Categorical vs Quantitative 
...
# Example 3: Categorical vs Categorical
